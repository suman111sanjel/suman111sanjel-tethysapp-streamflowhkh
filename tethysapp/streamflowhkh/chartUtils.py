...
from .config import AuthorizationToken,APIHost
...
def get_returnPeriod_data(comid):
    ...
    res = requests.get(APIHost+'/apps/apicenter/HKHapi/getreturnPeriodECMWFHKH', params=request_params, headers=request_headers)
...
def get_historic_data(comid):
    ...
    request_params = dict(comid=comid, cty='hkh')
    request_headers = dict(Authorization=AuthorizationToken)
    res = requests.get(APIHost+'/apps/apicenter/HKHapi/getHistoricECMWFHKH', params=request_params,headers=request_headers)
    data = json.loads(res.text)
    for key, value in data.items():
        if key == 'hdates':
            hdates = value
...
def get_flow_duration_curve(comid):
    request_params = dict(comid=comid, cty = 'hkh')
    request_headers = dict(Authorization=AuthorizationToken)
    res = requests.get(APIHost+'/apps/apicenter/HKHapi/getFlowDurationCurveECMWFHKH', params=request_params,headers=request_headers)
    data = json.loads(res.text)
    for key, value in data.items():
        if key == 'prob':
            prob = value
...
def getForecastData(comid):
    ...
    # url = APIHost + '/apps/apicenter/HKHapi/getForecastECMWFHKH'
    # request_params = {"comid": comid, 'cty': 'hkh'}
    # request_headers = {'Authorization': AuthorizationToken}
    # print("rr")
    # with requests.Session() as s:
    #     print('ii')
    #     res = s.get(url, params=request_params, headers=request_headers)
    #     print(res.text)
    #     # print(response.text)
    #     data = json.loads(res.text)

    res = requests.get(APIHost+'/apps/apicenter/HKHapi/getForecastECMWFHKH', params=request_params,headers=request_headers)
    data = json.loads(res.text)
    for key, value in data.items():
        if key == 'dates':
            dates = value
...
def get_forecastpercent(comid):
    ...
    rpall = requests.get(APIHost+'/apps/apicenter/HKHapi/getreturnPeriodECMWFHKH/',
    ...
    request_params = dict(cty='hkh', comid=comid)
    request_headers = dict(Authorization=AuthorizationToken)
    ens = requests.get(APIHost+'/apps/apicenter/HKHapi/getEnsembleCSVECMWFHKH/', params=request_params,
                       headers=request_headers)
